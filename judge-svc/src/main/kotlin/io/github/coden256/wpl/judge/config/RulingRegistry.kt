...
class ConfigurationPropertiesRulingRegistry(properties: RulingProperties)
    : RulingRegistry {
        val categories = properties.asRulingCategories()
    ...
    fun asRulingCategories(): Map<String, List<LawRuling>>{
        ...
        for ((categoryId, rulings) in rulings){
            ...
            for ((action, paths) in rulings) {
                ...
                for (path in paths){
                    category.add(LawRuling(path, Ruling(actualAction)))
                }
            }
